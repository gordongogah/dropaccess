...
const themeColor = () =>{
    ...
    hueSlider.addEventListener("input", function(){
        setHue(this.value);
        /*set the user's prefered in local storage */
        localStorage.setItem("--hue", this.value);
    });
    const slider =(value) =>{
        hueSlider.value=value;
    }
    /*check for saved user prefernce if any , on load of the website */
...
const themeLightDark = ()=>{
    ...
    darkModeCheckbox.addEventListener("click",function(){
        /*set the users prefrernce in local storage*/
        localStorage.setItem("theme-dark", this.checked);
        themeMode();
    });
    /*check for saved user prefrence, if, any, load of the website*/
